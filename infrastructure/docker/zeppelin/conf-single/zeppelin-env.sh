...
export SPARK_MASTER=spark://localhost:7077         # Spark master url. eg. spark://master_addr:7077. Leave empty if you want to use local mode.
export ZEPPELIN_ADDR=zeppelin                   # Bind address (default 127.0.0.1)
export ZEPPELIN_PORT=9010                       # port number to listen (default 8080)

export ZEPPELIN_LOCAL_IP=zeppelin               # Zeppelin's thrift server ip address, if not specified, one random IP address will be choosen.
...
export SPARK_CONF_DIR=/opt/spark/conf           # (optional) In the zeppelin interpreter on docker mode, Need to set the local spark conf folder path
