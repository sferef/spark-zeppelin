...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _comparative = __webpack_require__(8);
	...
	var Chart = function (_Visualization) {
	    ...
	    var spec = {
	      charts: {
	        'range': {
	          ...
	          parameter: _range.CommonParameter
	        },
	        ...
	        'comparative': {
	          ...
	          parameter: _comparative.ComparativeParameter
	        }
	      }
	    };
	  ...
	  _createClass(Chart, [{
	    ...
	    key: 'drawRangeChart',
	    value: function drawRangeChart(parameter, column, transformer) {
	      ...
	      var data = (0, _range.createRangeChartDataStructure)(rows, selectors, keyNames);
	      var chartOption = (0, _range.createRangeChartOption)(data, parameter, keyNames);

	      this.chartInstance = _highcharts2.default.chart(this.getChartElementId(), chartOption);
	    }
	  }, {
	    key: 'drawComparativeChart',
	    value: function drawComparativeChart(parameter, column, transformer) {
	      ...
	      var data = (0, _comparative.createComparativeChartDataStructure)(rows, selectors, keyNames);
	      var chartOption = (0, _comparative.createComparativeOption)(_highcharts2.default, data, parameter, keyNames);

	      this.chartInstance = _highcharts2.default.chart(this.getChartElementId(), chartOption);
	      this.keepYAxisCenterAsZero(this.chartInstance);
	    }
	  }, {
	    key: 'keepYAxisCenterAsZero',
	    value: function keepYAxisCenterAsZero(chart) {
...
/***/ (function(module, exports) {
	...
	exports.createRangeChartDataStructure = createRangeChartDataStructure;
	exports.createRangeChartOption = createRangeChartOption;
	...
	function createRangeChartDataStructure(rows, selectors, keyNames) {
	  var series = [];

	  // selectors.length always be even
	  for (var i = 0; i < selectors.length; i = i + 2) {
	    var splitedSelectorName = selectors[i].split('(');
	    var s = { name: splitedSelectorName[0], data: [] };
	    var reversed = false;
	    if (rows[i].selector.endsWith('(max)') && rows[i + 1].selector.endsWith('(min)')) {
	      reversed = true;
	    }

	    for (var j = 0; j < keyNames.length; j++) {
	      var minMaxRow = reversed ? [rows[i + 1].value[j], rows[i].value[j]] : [rows[i].value[j], rows[i + 1].value[j]];
	...
	function createRangeChartOption(data, parameter, keyNames) {
	  ...
	  var option = {
	    ...
	    tooltip: {
	      ...
	      pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' + ('<td style="padding:0"><b>' + getPrecisionFormat(tooltipPrecision, 'point.low') + '</b> ~ ') + ('<b>' + getPrecisionFormat(tooltipPrecision, 'point.high') + '</b></td></tr>'),
	  ...
	  if (yAxisName !== '') {
	    option.xAxis.title = { text: yAxisName /** inverted */ };
	  }
	  if (xAxisName !== '') {
	    option.yAxis.title = { text: xAxisName /** inverted */ };
	  }
	  if (yAxisUnit !== '') {
	    option.xAxis.labels = { format: '{value} ' + yAxisUnit /** inverted */ };
	  }
	  if (xAxisUnit !== '') {
	    option.yAxis.labels = { format: '{value} ' + xAxisUnit /** inverted */ };
	  }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.ComparativeParameter = undefined;
	exports.createComparativeChartDataStructure = createComparativeChartDataStructure;
	exports.createComparativeOption = createComparativeOption;

	var _range = __webpack_require__(7);

	var p = JSON.parse(JSON.stringify(_range.CommonParameter));
	p.dataLabel.defaultValue = false;
	var ComparativeParameter = exports.ComparativeParameter = p;

	function createComparativeChartDataStructure(rows, selectors, keyNames) {
	  var series = [];

	  // iterate selectors.length == 2, cause selector should be binomial in the comparative chart
	  var refinedSelectors = selectors.slice(0, 2);

	  for (var i = 0; i < refinedSelectors.length; i++) {
	    var s = { name: refinedSelectors[i], data: [] };

	    for (var j = 0; j < keyNames.length; j++) {
	      var value = rows[i].value[j];
	      if (i === 0) {
	        value = -value; /** the first series of comparative chart has - values */
	      }
	      s.data.push(value);
	    }
	...
	function createComparativeOption(Highcharts, data, parameter, keyNames) {
	  var xAxisName = parameter.xAxisName,
	      yAxisName = parameter.yAxisName,
	      xAxisUnit = parameter.xAxisUnit,
	      yAxisUnit = parameter.yAxisUnit,
	  ...
	  var option = {
	    ...
	    tooltip: {
	      ...
	      pointFormatter: function pointFormatter() {
	        var pointValue = parseNumberUsingHighchartPrecision(Highcharts, tooltipPrecision, this.y);
